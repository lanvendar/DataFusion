...
import { useVariableListQuery } from "./use-list-query";

interface VariableListTableProps {
  loading?: boolean;
  onAction: (action: PageActionEnum, record?: VariableItem) => void;
}

export function VariableListTable({ loading, onAction }: VariableListTableProps) {
  const { filter, setFilter, query, search, reset, setCurrent, setPageSize } = useVariableListQuery();
