...
- 同一任务实例重新提交时，Master 复用实例中的任务数据和插件配置，Worker 直接整体覆盖同内容快照；
- 插件执行器只读快照，不自行保存或改写；
...
`RunningTaskContext` 不进入长期缓存。`snapshot` 在创建后只读；`executionState` 是本次动作独占的候选副本，
插件可以设置状态、运行引用和退出信息。动作结束后由 Coordinator 合并 `WorkerResult` 并尝试 revision CAS，
Context 随即释放。
