...
3. 整体保存新的 .snap；Store 返回并复用该 taskInstanceId 的规范 workDirPath。
4. Coordinator 以当前 .state revision 准入并写入 SUBMITTING，同时清空上一轮运行引用。
5. 创建动作级 RunningTaskContext 并调用 executor.validate(context)；校验失败提交 `SUBMIT_FAILURE`。
6. 调用 executor.submit(context)。插件只创建第三方资源并返回 WorkerResult；K8S 插件按当前快照生成的确定性资源名幂等清理同名旧资源。
7. LOCAL 插件取得进程和 appId 后立即注册退出 watcher。
...
Master 重新提交时复用同一个任务实例 ID、已渲染任务数据和实例插件配置，因此新的 `.snap` 与上一轮提交一致。
DataX/Spark 使用当前快照重新计算确定性的 Kubernetes 资源名，先幂等清理同名旧资源，再创建新资源。
不保存历史快照或历史状态，也不增加时序钩子。
...
-> executor.stop/kill 返回 WorkerResult 或抛异常
-> Coordinator 提交动作结果
...
```text
读取 .snap/.state 并调用 executor.finish(context)
...
- 重提和崩溃恢复均依赖相同任务实例配置生成的确定性资源名执行幂等清理，不增加历史快照或历史状态。
