...
解析参数
    ...
    -> 创建日志目录
    -> 渲染 plugins/datax/templates/datax-local-runtime.yml 得到 LocalProcessSpec
    -> ProcessBuilder 启动 DataX Engine
    -> appId = process.pid()
    -> 写 WorkerTaskExecutionSnap(...)
    -> 写 WorkerTaskExecutionState(status=RUNNING, appId=pid, logPath=...)
...
解析参数
    ...
    -> 写 WorkerTaskExecutionSnap(...)
    -> 写 WorkerTaskExecutionState(status=RUNNING, appId=jobName, logPath=...)
```
