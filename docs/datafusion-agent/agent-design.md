...
读取 AgentProperties
    ...
    -> 装配 AgentTaskStateListenerRegistry
    -> 注册 worker 到 manager
    -> 按 workerId 从 manager 获取属于自己的未完成任务清单
    -> 按任务清单恢复本地 WorkerTaskExecutionState 并逐任务注册监听
    -> 启动心跳
...
`AgentExecutorRpcProvider` 只做 RPC 适配、ready 校验、监听注册和返回包装。任务执行和状态竞争语义进入
worker 框架。`HttpManagerClient` 负责 outbound RPC，`ManagerTaskResultReporter` 在当前调用线程直接上报，
不再二次提交到独立上报线程池。Manager HTTP client 必须配置连接和读取超时，防止监听线程长期阻塞。
...
TaskRequest(pluginType, runMode, taskData, pluginParam)
    ...
    -> WorkerTaskService 整体保存 WorkerTaskExecutionSnap，并 CAS 写入动作前置态
    -> PluginTaskExecutor.submitTask
    -> 插件 CAS 写入一次动作结果
    -> WorkerTaskService 重读 WorkerTaskExecutionState 作为响应结果
    -> 按 taskInstanceId 注册状态监听，并以本次 TaskResult.taskState 初始化 reportedStatus
...
- `.snap` 保存最近一次提交快照，包含 `workerId`、`pluginType`、`runMode`、`submitMode`、`taskData`、`pluginParam`；
  由 `WorkerTaskService` 在调用插件前整体覆盖，不做旧值读取、字段合并或对象复制。插件不构造或覆盖 `.snap`，
  `.snap` 也不参与运行态 revision 和任务状态锁。
...
- `WorkerTaskExecutionStore` 是 agent 本地运行态的唯一缓存入口；`executionCache` 保存当前运行上下文，
  状态监听是否存在只由 `AgentTaskStateListenerRegistry` 的 `taskInstanceId` 注册项决定。
...
- `saveState(state, expectedRevision)` 在任务锁内复读旧 `.state`；当前 revision 不等于预期值时记录
  `warn`、返回 `false` 且不写入，一致时将 `revision` 加一并写入。同时比较旧状态，当 `status`、`appId` 或
  `exitCode` 变化时追加 `state.log`，避免 watcher 或状态刷新器原地修改运行态对象导致终态漏记。
- `WorkerTaskExecutionContext` 为每个 `taskInstanceId` 创建进程内 `ReentrantLock`，但锁只封装在
  `saveState` 内部，不暴露给 Worker SPI 或调用方。`.state` 通过临时文件原子替换；单次读取、删除、
  Manager 上报和 `.snap` 读写都不持有该锁。锁不持久化，不处理多 Agent 写同一任务的场景。
- `WorkerTaskService` 在调用插件前，先用 revision CAS 将可执行的 submit/stop/kill 写为
...
- 插件完成 submit/stop/kill 后使用动作前读取的 revision 提交一次结果；`WorkerTaskService` 不再按插件返回值二次写入，
  而是重读 `.state` 返回权威状态。若 watcher 已抢先写入终态，终态自然覆盖 `SUBMIT_SUCCESS`。
- RPC 层在 `WorkerTaskService` 返回后注册周期监听；Shell/DataX LOCAL 的进程 watcher 也必须在提交状态保存后启动，
  不引入额外的提交后钩子。
...
submit/stop/kill 或启动恢复
    ...
    -> 调用 PluginRunModeStateMapping.mapState(snapshot, Q) 查询第三方并映射状态
    -> 非终态的映射状态等于 Q.status 且等于 reportedStatus 时直接结束
    -> 校验状态迁移，终态补偿上报前调用 prepareFinalReport(snapshot, state)
    -> 状态或终态结果变化后调用 saveState(state, Q.revision)
...
路由、参数恢复、状态映射和终态准备，`.state` 的状态与终态结果均通过同一个 revision CAS 提交。
...
`finishTask` 不受保留策略影响，始终注销监听、执行插件清理并删除实际存在的 `.snap/.state`；
状态文件删除和 `executionCache` 失效不占用 `saveState` 内部锁。
...
register 成功返回 Worker.id
    ...
    -> 扫描一次本地 .state 并建立 taskInstanceId 到任务目录索引
...
任务清单查询或恢复失败时 Agent 保持未就绪，下一次继续走注册恢复；恢复成功后的 heartbeat 不再重复扫描。
