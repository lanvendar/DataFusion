...
| `DataxPluginTaskExecutor` | 多模式公共执行逻辑 | 请求适配、快照、状态持久化、结果组装 | abstract class | Agent 进程 | LOCAL / K8S 执行器共用 |
...
| `result` | `WorkerTaskExecutionState` | `JsonNode` | Java object | 执行器 / 状态映射写入 | 不包含完整 job JSON、密码和本地退出码；上报时映射为 `TaskResult.workerResult` |
