...
`pluginParam` 是插件级运行配置，来源于 Manager 侧 `system_plugin_config.plugin_param`，并由 Manager 把 `PluginConfigEntity.runMode` 注入到 `pluginParam.runMode`。Agent 提交时不从 `taskData.runMode` 或配置文件推断运行模式，也不回写 `pluginParam`。
...
| `DataxExecutionParam` | 归一化执行参数 | `runMode`, `taskInstanceId`, `flowInstanceId`, `jobFile`, `jobJson`, `effectiveTaskData`, `workDir`, `logFile`, `dataxHome`, `dataxJar`, `jvmOptions`, `kubernetes` | Java object | 单次任务 | 从 `TaskRequest` 归一化；runner 只消费该对象，不直接承载调度请求 |
...
| `DataxTaskRunner` | 运行模式分派 | `runMode()`, `submit(param)`, `stop(param, state)`, `kill(param, state)`, `finish(param, state)` | interface | Spring Bean | LOCAL / K8S Runner 只处理 DataX 执行参数 |
| `DataxTaskResult` | Runner 返回 | `status`, `appId`, `workDirPath`, `result`, `kubernetesRuntimeRef` | Java object | 单次动作 | 由 `DataxPluginTaskExecutor` 转为 `TaskResult.workerResult`；插件日志入口从 `result.pluginLogUri` 映射；K8S runtime ref 用于持久化接管信息 |
...
| `result` | `WorkerTaskExecutionState` | `JsonNode` | Java object | Runner / 状态映射写入 | 不包含完整 job JSON、密码和本地退出码；上报时映射为 `TaskResult.workerResult` |
...
| `TaskRequest.pluginParam` + `TaskRequest.taskData` -> `DataxExecutionParam` | `taskData` 覆盖任务级字段，`pluginParam` 提供插件级字段；缺失字段只使用代码内协议默认值 | `jobJson` 不写日志；控制请求恢复时只临时用 `.snap.runMode` 补足解析参数 |
...
| K8S kill -> Kubernetes resources | 删除 Job 和本次任务 Secret；无 Job 引用视为无残留 | 成功/无资源返回 `KILLED`；失败返回 `UNKNOWN`；finish 仍会幂等兜底清理 |
| LOCAL 退出码 -> `StatusEnum` | `0 -> RUN_SUCCESS`，非 `0 -> RUN_FAILURE` | stop/kill 已写终态时 watcher 不覆盖 |
| K8S Job / Pod status -> `StatusEnum` | `Complete -> RUN_SUCCESS`，`Failed -> RUN_FAILURE`，`active > 0 -> RUNNING`，stop 后 Pod 全部退出 -> `STOP_SUCCESS`，kill 后 Pod 全部退出 -> `KILLED`，找不到且未终态 -> `UNKNOWN` | stop 先写 `STOPPING`；kill 清理成功直接写 `KILLED` |
