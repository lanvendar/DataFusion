...
| `DataxSubmitResult` | Runner 提交返回 | `status`, `appId`, `workDirPath`, `result`, `kubernetesRuntimeRef` | Java object | 单次提交 | 由 `DataxPluginTaskExecutor` 转为 `TaskResult.workerResult`；插件日志入口从 `result.pluginLogUri` 映射；K8S runtime ref 用于持久化接管信息 |
