...
| `DataxTaskRunner` | 运行模式分派 | `runMode()`, `submit(param)`, `stop(param, state)`, `kill(param, state)` | interface | Spring Bean | LOCAL / K8S Runner 只处理 DataX 执行参数 |
...
| K8S submit 前置清理 | `.state.appId` 存在时，按 Job name 清理旧 Job、匹配 Pod 和 Secret，清理完成后再提交；清理失败返回 `SUBMIT_FAILURE` | 用于同一 taskInstanceId 重试，避免旧失败 Pod 或同名 Job 阻塞重新提交 |
