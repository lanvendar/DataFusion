...
| `DataxExecutionParam` | 归一化执行参数 | `runMode`, `taskInstanceId`, `flowInstanceId`, `jobFile`, `jobJson`, `effectiveTaskData`, `workDir`, `logFile`, `dataxHome`, `dataxJar`, `jvmOptions`, `kubernetes` | Java object | 单次任务 | 从 `RunningTaskContext.snapshot` 归一化 |
...
| `DataxPluginTaskExecutor` | 多模式公共执行逻辑 | 参数解析和第三方动作辅助 | abstract class | Agent 进程 | LOCAL / K8S 执行器共用；不读写 Store、不构造 `TaskResult` |
| `LocalDataxPluginTaskExecutor` / `K8sDataxPluginTaskExecutor` | 模式执行器 | `LOCAL` / `K8S` | Spring Bean | Agent 进程 | 按 `DATAX + runMode` 独立注册 |
| `WorkerResult` | 执行结果 | `appId`, `workDirPath`, `message`, `pluginLogUri`, `outputVars` | Java object | 单次动作 | 复用 common-data 既有结构；状态由 Coordinator 决定 |
...
| `appId` | `WorkerTaskExecutionState` | LOCAL: pid；K8S: Job name | String | Coordinator 合并插件 `WorkerResult` 后写入 | 终端任务 ID |
| `workDirPath` | `WorkerTaskExecutionState` | 任务运行目录 | String | Coordinator 合并插件 `WorkerResult` 后写入 | Agent 重启后周期上报仍保留任务运行目录；插件日志入口保存在 `result.pluginLogUri` |
...
| `result` | `WorkerTaskExecutionState` | `JsonNode` | Java object | Coordinator 把 `WorkerResult` 转为动作摘要；映射器准备终态候选后统一持久化 | 不包含完整 job JSON、密码和本地退出码；上报时映射为 `TaskResult.workerResult` |
...
| `RunningTaskContext` / `WorkerResult` | `datafusion-scheduler-worker` / `datafusion-common-data` | 动作级输入与公共插件结果 |
| `WorkerTaskExecutionState` / `WorkerTaskExecutionStore` / `WorkerTaskStateCoordinator` | `datafusion-scheduler-worker` | Agent 本地状态记录和唯一状态迁移入口 |
