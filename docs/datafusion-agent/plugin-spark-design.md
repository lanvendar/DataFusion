...
Manager scheduler
    ...
    -> WorkerService
    -> WorkerPluginRouter(SPARK + K8S_OPERATOR)
...
`WorkerPluginRouter` 按 `SPARK + K8S_OPERATOR` 选择执行器和状态映射器。
...
| 动作 | 行为 | 返回 |
|------|------|------|
| `submit` | 写本地 job 文件，创建 ConfigMap 和 SparkApplication | 插件返回 `WorkerResult`；Coordinator 成功写 `SUBMIT_SUCCESS`，异常写 `SUBMIT_FAILURE` |
