...
| `logPath` | `WorkerTaskExecutionState` | 插件日志入口 | String | Runner 提交后写入 | Agent 重启后周期上报仍保留插件日志入口；优先与 `TaskResult.result.pluginLogUri` 一致 |
...
| `DataxExecutionParam` -> LOCAL job file | `pluginParam.jobFile` 非空时复制到 `${modules}/${taskRuntimeDir}/{date}/{flowInstanceId}/{taskInstanceId}/job.json`；否则把 `jobJson` 或 `effectiveTaskData` 写入该文件 | 已提交任务保留本次 job 快照 |
...
| `DataxExecutionParam` -> K8S Secret | 先把 `jobJson` 或 `effectiveTaskData` 写入 `${modules}/${taskRuntimeDir}/{date}/{flowInstanceId}/{taskInstanceId}/job.json`，再读取该快照写入 Secret key `job.json` 并挂载到 `jobJsonMountPath` | DataX JSON 可能含密码，默认不用 ConfigMap |
...
| `agentLogPath` | `String` | 否 | Agent 自身日志或状态入口，只指向 `/opt/datafusion/datafusion-agent/...` 一类路径；没有明确路径时为空 |
