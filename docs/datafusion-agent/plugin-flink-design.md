...
- agent ServiceAccount 有目标 namespace 内 Pod、Pod log 和 `flinkdeployments.flink.apache.org` 的必要权限。
...
- 提交成功后只把 `FlinkDeployment` name 写入 `.state.appId`；namespace、podLabelSelector 等运行引用
...
- ServiceAccount 最小权限：目标 namespace 内 Pod、Pod log 和 `flinkdeployments.flink.apache.org`。
