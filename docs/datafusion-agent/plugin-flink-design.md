...
- 共享 PVC 中单个 Flink app 目录由 `pluginParam.flinkAppDir` 直接指定，例如
  `/opt/datafusion/plugins/flink/datafusion-plugin-kafka-json`。
...
- 首版不二次制作业务 Flink 镜像。Agent 在提交 `FlinkDeployment` 时渲染 `spec.podTemplate`，注入共享盘 PVC、
  `/opt/flink/usrlib` `emptyDir`、job config Secret mount 和 initContainer；Operator 按该 podTemplate 创建
...
解析参数
    ...
    -> 读取任务运行目录/flink-job.json 生成 Secret data.flink-job.json
...
- `flink-job.json` 先落盘到 `${taskRuntimeDir}/{date}/{flowInstanceId}/{taskInstanceId}/flink-job.json`，
...
- Secret 通过 `podTemplate` 挂载到 Flink JobManager / TaskManager container，默认容器内路径为
  `/opt/datafusion/task-runtime/{yyyyMMdd}/{flowInstanceId}/{taskInstanceId}/flink-job.json`。
- 用户提供的 `taskData.kubernetes.podTemplate` 只允许追加 labels、annotations、env、nodeSelector、
  ...
  names、job config Secret mount 和 usrlib `emptyDir`。首版可以先禁用用户 podTemplate，或只实现 allowlist 合并。
- 作业参数默认传入
  `--config /opt/datafusion/task-runtime/{yyyyMMdd}/{flowInstanceId}/{taskInstanceId}/flink-job.json`。
...
builder 只负责输出 `plugins/flink/datafusion-plugin-kafka-json/`；不得清空 `plugins/flink/` 根目录。单个 app 的主
...
| `datafusion-plugin/datafusion-plugin-kafka-json/src/main/resources/plugins/plugin-kafka-json-commands.md` | 当前仍有旧样例路径描述，后续需和 `plugins/flink/{appDirName}/jobs` 结构对齐 |
