...
- 编译脚本只负责把 `datafusion-plugin-kafka-json` 产物部署到
  `datafusion-agent/src/main/resources/plugins/flink/datafusion-plugin-kafka-json/`；
  后续由外部流程把整个 `plugins/` 目录整体上传到共享 PVC。
...
解析参数
    ...
    -> 校验 sharedFlinkAppUri 下 flinkAppMainJar 存在
    -> 校验 sharedFlinkAppUri 的 app 根目录/lib 目录和 artifactMode 匹配
    -> 校验 image/buildFlinkVersion/flinkVersion/operatorVersion 版本组合匹配
...
- `runMode` 配置值使用小写值；agent 内部状态文件统一写枚举名大写。
- `K8S_OPERATOR` 镜像默认使用 `flink:2.2.0-scala_2.12-java17`，平台固定 `linux/amd64`，`flinkVersion=v2_2`。
- `K8S_OPERATOR` 必须提供 `operatorVersion=flink-kubernetes-operator:1.14.0`、`flinkVersion`、`buildFlinkVersion`、`artifactMode`、`mavenProfile`、`entryClass`、`flinkAppName`、`flinkAppDir`、`flinkAppMainJar`、`agentFlinkAppDir`、`sharedPvcName`、`sharedFlinkAppUri` 和 `usrlibPath`；`jarURI` 可由 `usrlibPath + flinkAppMainJar` 派生。
- `flinkAppDir` 使用 `/opt/datafusion/plugins/flink/{flinkAppName}`；`/opt/flink/lib` 是 Flink runtime 自带目录，
...
- `flinkConfiguration` 按 `pluginParam.flinkConfiguration -> pluginParam.kubernetes.flinkConfiguration -> taskData.flinkConfiguration -> taskData.kubernetes.flinkConfiguration` 顺序合并，后者覆盖前者。
...
`datafusion-plugin-kafka-json` 子模块增加 builder 入口：
...
- Maven profile 负责产物语义：`flink-fat-jar` 产出 executable fat jar，`flink-thin-lib` 产出 thin jar +
  runtime dependencies。
- Shell builder 只做入口编排：选择 profile、创建并清空
  `datafusion-agent/src/main/resources/plugins/flink/datafusion-plugin-kafka-json/`、
  调用 Maven、复制产物、校验目录结构。
- 不建议纯 shell 实现依赖解析；依赖 scope、shade、copy-dependencies 必须由 Maven 管。
- 不建议把复制到 agent resources 绑定到普通 `package` 生命周期，避免开发者执行普通构建时意外修改
  `datafusion-agent/src/main/resources/plugins/flink/{flinkAppName}/`。如使用纯 Maven，也必须挂在显式 profile/goal 上。
- builder 脚本放在 `src/main/resources/builder/`，不要放进 `src/main/resources/plugins/{pluginType}/...`，
  避免构建脚本被误上传到共享盘插件运行目录。
- 当前 `datafusion-agent/src/main/resources/plugins/` 还没有 `flink` 目录时，首版 builder 负责创建
  `plugins/flink/{flinkAppName}/` 和 `lib/`。
- `datafusion-plugin/datafusion-plugin-kafka-json/src/main/resources/plugins/flink/` 已经按 Flink 类型目录组织；
  后续应调整为 `plugins/flink/datafusion-plugin-kafka-json/`，便于和 agent 发布目录一一对应。
- builder 只允许清理和覆盖自己的 `{flinkAppName}` 子目录，不允许清空 `plugins/flink/` 根目录，避免覆盖
  `datafusion-plugin-flink-sql` 等其他 Flink app。

builder 输出规范：

```text
datafusion-agent/src/main/resources/plugins/flink/datafusion-plugin-kafka-json/
  datafusion-plugin-kafka-json-1.0.0-executable.jar   # FAT_JAR
  lib/                                                # FAT_JAR 为空

datafusion-agent/src/main/resources/plugins/flink/datafusion-plugin-kafka-json/
  datafusion-plugin-kafka-json-1.0.0.jar              # THIN_JAR_LIB
  lib/*.jar                                           # THIN_JAR_LIB 依赖包
```

builder 的职责到 agent resources 目录为止。共享盘发布由后续外部流程统一打包
`datafusion-agent/src/main/resources/plugins/`，并上传到 PVC，例如形成：

```text
/opt/datafusion/plugins/flink/datafusion-plugin-kafka-json/{flinkAppMainJar}
/opt/datafusion/plugins/flink/datafusion-plugin-kafka-json/lib/*.jar
```

### 6.2 插件目录 manifest 规范

`plugins/{pluginType}` 根目录建议提供插件类型 manifest，用于说明该类型是否支持多个 app 发布包。
首版建议字段如下：
...
`multiApp=true` 表示该插件类型目录下可以有多个 app 子目录。Flink 使用该模式，以便
`datafusion-plugin-kafka-json`、`datafusion-plugin-flink-sql` 等模块共同汇聚到
`datafusion-agent/src/main/resources/plugins/flink/`，但互不覆盖。

`DataX` 这类大概率只有一种发布包的插件类型可以声明 `multiApp=false`，其目录继续保持
`plugins/datax/` 单包形态，不强制引入 app 子目录。

单个 Flink app 目录建议提供 app manifest，例如：
...
该 manifest 只服务构建、发布、校验和 Flink 提交，不改变 agent 公共 `pluginType` / `runMode` 模型。
...
| `datafusion-plugin/datafusion-plugin-kafka-json/pom.xml` | 增加 `flink-fat-jar`、`flink-thin-lib` profile，并按 6.4 调整 `provided` 和发布依赖 |
...
| `datafusion-plugin/datafusion-plugin-datax/Dockerfile` | 当前仍引用旧路径 `src/main/resources/datax/`，后续需改为 `src/main/resources/plugins/datax/` |
| `datafusion-plugin/datafusion-plugin-datax/src/main/resources/plugins/plugin-datax-commands.md` | 当前仍有 `resources/datax`、`resources/job` 等旧路径描述，后续需和 `plugins/datax` 结构对齐 |
...
- `runMode=k8s_operator` / `runMode=k8s` 解析为内部大写枚举。
...
- builder 脚本能按 `FAT_JAR` / `THIN_JAR_LIB` 输出
  `datafusion-agent/src/main/resources/plugins/flink/{flinkAppName}/` 规范目录。
- builder 脚本位于 `datafusion-plugin/datafusion-plugin-kafka-json/src/main/resources/builder/`，不会被复制到
  `plugins/flink/{flinkAppName}/` 运行目录。
- `plugins/flink/plugin-manifest.json` 声明 `multiApp=true`；单个 app 目录声明 `flink-app-manifest.json`。
- builder 只清理当前 `{flinkAppName}` 子目录，不清空 `plugins/flink/` 根目录。
