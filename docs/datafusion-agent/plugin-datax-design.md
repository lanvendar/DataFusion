...
`DATAX + K8S`，共同继承负责请求适配、状态持久化和结果组装的 `DataxPluginTaskExecutor`。完整提交快照由
`WorkerTaskService` 保存，DataX 执行器不再构造 `.snap`。
...
解析参数
    ...
    -> CAS 写 WorkerTaskExecutionState(status=SUBMIT_SUCCESS, appId=pid, workDirPath=任务运行目录,
       result.pluginLogUri=local-datax.log)
    -> watcher 等待退出码并更新 RUN_SUCCESS / RUN_FAILURE
```

进程 watcher 只在提交状态保存完成后启动，不使用 `afterSubmitStateSaved` 一类扩展钩子。
...
解析参数
    -> 生成 Job / Secret 名称
    -> 如果旧 .state.appId 存在, 按 BEFORE_SUBMIT 清理同名 Job、匹配 Pod 和 Secret；清理失败则返回 SUBMIT_FAILURE
    ...
    -> CAS 写 WorkerTaskExecutionState(status=SUBMIT_SUCCESS, appId=jobName, workDirPath=任务运行目录,
       result.pluginLogUri=插件日志入口)
```
...
- 重新提交前如果 `.state.appId` 存在，agent 会先按 `BEFORE_SUBMIT` 幂等清理旧 Job、匹配 Pod 和 Secret；
  只有清理完成才创建新 Job，清理失败直接返回 `SUBMIT_FAILURE` 并写入失败原因。
