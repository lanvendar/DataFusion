...
| `scheduler_worker_registry` | 新增 | `id uuid` | `worker_code`、`host + port` | 无 | 调度 worker 注册表，记录 worker 的注册状态、心跳时间、插件能力和运行元信息 | manager 侧 `WorkerStorageImpl` 的持久化表 |
...
| `status` | `int4` | `status` | `Integer` | 是 | `1` | 无 | 状态：0-下线 1-上线 2-清除 | 对应 `Worker.STATUS_DOWN/STATUS_UP/STATUS_DELETED` |
...
| `TaskResult` | `Action` | agent 上报任务结果 | `taskInstanceId`、`taskState`、`workerId`、`appId`、`result` | 多类型 | `taskState` 必填 | 由 `MasterService.getTaskAction().asyncHandle` 推进状态机 |
...
|-------------|--------------|--------|--------------|---------------|-----------------|-------|
| `status` | `Worker` / `scheduler_worker_registry` | `0` / `1` / `2` | `int4` | 下线 / 上线 / 清除 | 直接映射 `Worker.STATUS_DOWN`、`Worker.STATUS_UP`、`Worker.STATUS_DELETED` | 调度查找仅使用 `status = 1` |
