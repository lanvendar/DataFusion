...
| `SchedulerInstanceArchiveBatch` | 成功实例归档 | `batchSize`、成功状态集合、流程调度状态、流程定义版本匹配结果 | `Integer/List<String>/Boolean` | 定时任务内 | 成功状态集合来自 `StatusEnum.isSuccess()`；仅保护 `enabled=true` 且定义版本匹配的最新调度实例，其余成功实例不再保留实时记录 |
...
| `FlowInstanceHisEntity` / `TaskInstanceHisEntity` | 归档定时任务 | realtime DB -> his DB | 与实时表一致 | 与实时表一致 | 实时表记录状态满足 `StatusEnum.isSuccess()` 后复制到对应历史表，再删除实时表记录；仅当流程 `enabled=true` 且定义版本匹配时保留最新调度实例 | 成功实例归档 |
