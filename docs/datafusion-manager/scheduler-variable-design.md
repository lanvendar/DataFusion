# scheduler-variable 设计文档

> 数据结构定义见 [scheduler-variable-data-define.md](./scheduler-variable-data-define.md)，本文档不重复字段定义。

## 1. 能力说明

- 能力: 调度变量配置的分页查询、列表查询、新增、修改、详情查询和删除。
- 所属模块: `datafusion-manager`
- 包路径: `com.datafusion.manager.scheduler`
- 路径前缀: `/api/scheduler/variable`
- 调用链: `VariableController` -> `VariableInfoService` -> `VariableInfoServiceImpl` -> `VariableInfoMapper` -> `scheduler_variable_info`
...
| 文件 | 说明 |
|------|------|
| `docs/datafusion-manager/scheduler-variable-data-define.md` | 调度变量字段、类型、校验和层间映射定义 |
| `docs/datafusion-manager/scheduler-variable-design.md` | 调度变量 API、Service、Mapper、集成和验证设计 |
...
| 对象 | 路径 | 说明 |
|------|------|------|
| `VariableController` | `datafusion-manager/src/main/java/com/datafusion/manager/scheduler/controller/VariableController.java` | HTTP 入口 |
| `VariableInfoService` | `datafusion-manager/src/main/java/com/datafusion/manager/scheduler/service/VariableInfoService.java` | Service 契约 |
| `VariableInfoServiceImpl` | `datafusion-manager/src/main/java/com/datafusion/manager/scheduler/service/impl/VariableInfoServiceImpl.java` | 业务实现 |
| `VariableInfoMapper` | `datafusion-manager/src/main/java/com/datafusion/manager/scheduler/dao/VariableInfoMapper.java` | MyBatis-Plus Mapper |
| `VariableInfoEntity` | `datafusion-manager/src/main/java/com/datafusion/manager/scheduler/po/VariableInfoEntity.java` | 表实体 |
...
| Scheduler runtime | 数据配置 | 当前接口只维护变量配置，未实现变量解析或任务引用检查 |
