# datafusion-plugin-flink-table 设计

> 数据结构定义见 [flink-table-data-define.md](flink-table-data-define.md)。本文档只描述 Kafka JSON 到 Paimon 的数据流、JMESPath 解析策略、主键策略、代码边界与验证计划。
...
- Module: `datafusion-plugin/datafusion-plugin-flink-table`
- Module boundary: 该模块是独立 Flink Table 插件运行时，负责从 Kafka JSON 解析记录并写入 Paimon。
- Java backend package: `com.datafusion.plugin.flink.table`
- Main class: `com.datafusion.plugin.flink.table.FlinkTablePaimonApplication`
- Frontend path: 无
- Route / API prefix: 无
- Call chain: `job config` -> `KafkaSource` -> `KafkaRecordDeserializer` -> `TableResolver` -> `JMESPath extractor` -> `RecordNormalizer` -> `Paimon Sink V2 writer` -> `Paimon committer`

## 2. Data Flow

| Scenario | Source | Through | Target | Data structure | Notes |
|----------|--------|---------|--------|----------------|-------|
| 启动作业 | 本地 `job.json` 或启动参数配置文件 | `ConfigLoader` -> `ConfigValidator` | Flink 作业运行时 | `FlinkTableJobConfig` | 启动前完成配置和表达式校验 |
| 消费 Kafka 消息 | Kafka topic | Flink `KafkaSource<KafkaRecord>` | `KafkaRecordDeserializer` | 原始 JSON 字符串和 Kafka 元信息 | 不要求固定 envelope |
| 表路由解析 | message object + `sink.tables[]` | `TableResolver` | `PaimonTableConfig` | 命中的目标表配置 | `table.database` 和 `table.name` 必须由 job 静态配置；Kafka JSON 中 `schema.table.name` 等于 `table.name` 时命中；无表名或未命中时过滤消息 |
| 解析记录数组 | message object + `columnsMapping` | `ExpressionEvaluator` | record objects | `Array<Object>` | `columnsMapping.path` 是消息级 JMESPath，结果支持对象数组或单层对象，单层对象自动包装为一条记录 |
| 解析目标表结构 | message object + `PaimonTableConfig` | `TableResolver` | `ResolvedTableConfig` | database/tableName/table metadata/columns/options | `table.database/name` 来自 job；真实 Paimon 表存在时结构优先；缺表建表时优先使用 job 完整定义，job 未配置 `columns[]` 时字段可取 Kafka `schema.columns[]` |
| 解析单条记录 | record object + `columns[]` | `TableResolver` | `Map<String, Object>` | 写入记录 | 每列按 `columns[].value` 取值和兜底 |
| 生成代理主键 | mapped record + `PrimaryKeyConfig` | `ProxyPrimaryKeyGenerator` | mapped record | `_id_` | `PROXY` 模式自动补充固定代理主键列；`mode` 未配置时默认 `FIELDS` |
...
`job.json` 是目标表路由和建表配置源。Kafka 消息可携带标准结构 `schema.table` / `schema.columns`，其中 `schema.table.name` 用于与 `sink.tables[].table.name` 做路由匹配，`schema.columns[]` 只在真实 Paimon 表不存在且 job 未配置 `columns[]` 时作为建表字段定义兜底。`table.database` 和 `table.name` 必须由 job 静态定义；字段定义不能局部覆盖，job 中一旦配置 `columns[]` 就必须是完整字段定义。
...
```json
{
  ...
  "source": {
    ...
    "groupId": "datafusion-plugin-flink-table",
  ...
  "flinkConfig": {
    ...
    "execution.checkpointing.dir": "file:///tmp/flink-checkpoints/flink-table-oilchem-paimon",
...
| File | Notes |
|------|-------|
| `datafusion-plugin/datafusion-plugin-flink-table/pom.xml` | Maven 模块定义，artifactId 为 `datafusion-plugin-flink-table` |
| `datafusion-plugin/datafusion-plugin-flink-table/src/main/java/com/datafusion/plugin/flink/table/FlinkTablePaimonApplication.java` | 插件启动入口 |
| `datafusion-plugin/datafusion-plugin-flink-table/src/main/java/com/datafusion/plugin/flink/table/config/*` | 配置模型、加载和校验 |
| `datafusion-plugin/datafusion-plugin-flink-table/src/main/java/com/datafusion/plugin/flink/table/expression/*` | JMESPath 表达式编译、求值和类型校验 |
| `datafusion-plugin/datafusion-plugin-flink-table/src/main/java/com/datafusion/plugin/flink/table/resolve/*` | 表路由、字段映射、主键解析 |
| `datafusion-plugin/datafusion-plugin-flink-table/src/main/java/com/datafusion/plugin/flink/table/source/*` | Kafka source 构建和反序列化 |
| `datafusion-plugin/datafusion-plugin-flink-table/src/main/java/com/datafusion/plugin/flink/table/sink/*` | Paimon writer、schema 校验、批量提交 |
| `datafusion-plugin/datafusion-plugin-flink-table/src/main/resources/plugins/flink/jobs/example-flink-table-paimon-simple-job.json` | 标准结构简化样例，Kafka 消息提供 `schema.table` / `schema.columns` |
| `datafusion-plugin/datafusion-plugin-flink-table/src/main/resources/plugins/flink/jobs/example-flink-table-paimon-standard-job.json` | 通用结构样例，job 配置完整定义 table、columns 和 JMESPath |

### 4.1 Related References
...
|--------|-------|--------|-------|
| `load(String configPath)` | 配置文件路径 | `FlinkTableJobConfig` | 加载 JSON 配置并解析环境变量 |
| `validate(FlinkTableJobConfig config)` | 顶层配置 | `void` | 校验 Kafka、`flinkConfig`、sink、JMESPath 表达式和 Paimon 表结构配置 |
| `parse(String message)` | Kafka 原始消息 | `Object` | 使用 Jackson 解析任意 JSON |
| `evaluate(ExpressionSpec spec, Object context)` | 表达式配置和上下文 | `Object` | JMESPath 求值、默认值兜底和 `jsonType` 顶层校验 |
| `resolveTable(Object message, List<PaimonTableConfig> tables)` | Kafka 消息和表配置 | `Optional<PaimonTableConfig>` | 使用静态 `table.name` 和 Kafka `schema.table.name` 匹配目标表 |
| `resolve(KafkaRecord record)` | Kafka 消息和 Kafka 元信息 | `Optional<ResolvedTableWritePlan>` | 通过 `columnsMapping` 生成单表写入计划 |
...
|----------|------|----------------|
| Kafka 消息不是合法 JSON | `TableResolver` 解析失败 | `recordErrorPolicy=SKIP` 时记录 WARN 并跳过当前 Kafka record；`FAIL` 时抛异常 |
...
| 多表路由 | `sink.tables[].table.name` 是静态表名；Kafka `schema.table.name` 与静态表名相等时命中 | 未命中或为空时记录 WARN 并跳过，日志包含 topic、partition、offset、tableName、reason |
...
| JMESPath | `io.burt:jmespath-core` | 负责用 `columnsMapping` 从消息投影记录数组，从消息中解析表元信息，并从单条记录中解析字段值 |
...
- Unit tests: `ExpressionSpec` 求值、`jsonType` 校验、静态 `table.name` 路由、`columnsMapping` 解析、record 映射、代理主键生成、配置校验。
- Compile / build command: `mvn -DskipTests compile -pl datafusion-plugin/datafusion-plugin-flink-table -am`。
