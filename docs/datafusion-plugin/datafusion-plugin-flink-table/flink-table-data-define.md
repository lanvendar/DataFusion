# datafusion-plugin-flink-table 数据定义

> 本文档是 `datafusion-plugin-flink-table` 的数据结构定义，覆盖 Kafka JSON 解析配置、JMESPath 表达式取值、Paimon 表结构、主键策略与运行时写入计划。
...
|--------|----------|-------|------------|-----------|-------|
| `FlinkTableJobConfig` | 插件启动加载整份任务配置 | `job` | `JobConfig` | 启动时加载，运行期只读 | 作业元信息 |
| `FlinkTableJobConfig` | 插件启动加载整份任务配置 | `source` | `KafkaSourceConfig` | 启动时加载，运行期只读 | Kafka 消费配置 |
| `FlinkTableJobConfig` | 插件启动加载整份任务配置 | `flinkConfig` | `Map<String, String>` | 启动时加载，运行期只读 | Flink 官方配置 key/value，直接透传给 Flink `Configuration` |
| `FlinkTableJobConfig` | 插件启动加载整份任务配置 | `sink` | `PaimonSinkConfig` | 启动时加载，运行期只读 | Paimon catalog、表解析和写入配置 |
...
`flinkConfig` 只做 Map 结构校验，不维护插件自定义字段。
...
| `ExpressionSpec` | JMESPath | Inbound | `path` | `String` | 基于 Java object 编译和求值 | 使用 `io.burt:jmespath-core` |
...
|-----------|-----------------|------------------|
| Kafka raw JSON -> Java object | `TableResolver` 使用 Jackson 解析整条 Kafka value | 非法 JSON 由 `recordErrorPolicy` 或作业失败策略处理 |
...
| Kafka JSON + `PaimonTableConfig` -> `ResolvedTableConfig` | 使用静态 `sink.tables[].table.name` 与 Kafka `schema.table.name` 匹配目标表 | 多表按 `tables[]` 顺序解析，第一张命中的配置生效；不支持一条消息写多张表 |
...
| Object | Path | Notes |
|--------|------|-------|
| Kafka/Flink/Paimon 配置模型 | `datafusion-plugin/datafusion-plugin-flink-table/src/main/java/com/datafusion/plugin/flink/table/config/FlinkTableJobConfig.java` | 顶层 job、source、flinkConfig、sink、table、columns、primaryKeys、writer 配置 |
| JMESPath 表达式 | `datafusion-plugin/datafusion-plugin-flink-table/src/main/java/com/datafusion/plugin/flink/table/expression` | 表达式归一化、求值和顶层 JSON 类型校验 |
| 表解析和写入计划 | `datafusion-plugin/datafusion-plugin-flink-table/src/main/java/com/datafusion/plugin/flink/table/resolve` | 标准 schema 解析、静态表路由、字段映射、代理主键生成 |
| Paimon sink | `datafusion-plugin/datafusion-plugin-flink-table/src/main/java/com/datafusion/plugin/flink/table/sink` | schema cache、schema 校验、writer 缓存、批量写入和提交 |
| Kafka source | `datafusion-plugin/datafusion-plugin-flink-table/src/main/java/com/datafusion/plugin/flink/table/source` | Kafka source 构建、record 反序列化和 topic/partition/offset 元信息保留 |
