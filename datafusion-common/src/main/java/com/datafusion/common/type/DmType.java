...
public class DmType extends AbstractType {
    /**
     * 字段类型定义.
    ...
    protected void defineMappings(Map<DataType, Pair<String[], Boolean>> map) {
        ...
        map.put(DataType.BIGINT, Pair.of(new String[] {"LONG", "BIGINT"}, true));
        map.put(DataType.DECIMAL, Pair.of(new String[] {"NUMBER", "DECIMAL", "DEC"}, true));
        ...
        map.put(DataType.VARCHAR, Pair.of(new String[] {"VARCHAR2", "NVARCHAR"}, true));
        ...
        map.put(DataType.BINARY, Pair.of(new String[] {"BIT"}, true));
    ...
    public TypeInfo getDefaultType() {
        return TypeInfo.builder().dataType(DataType.VARCHAR).javaType(String.class).fieldType("VARCHAR2").length(1000).precision(null).scale(null)
                .build();
    }
