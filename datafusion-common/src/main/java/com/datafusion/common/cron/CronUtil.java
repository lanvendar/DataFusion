 ...
 * 思路：1、切割cron表达式 2、转换每个域 3、计算执行时间点（关键算法，解析cron表达式） 4、计算某一天的哪些时间点执行</p>
 *
 * <p>
 * 二、根据cron表达式，给定Date，计算下一个执行时间点
 * 思路：1、找到所有时分秒的组合并按照时分秒排序 2、给定的时分秒在以上集合之前、之后处理 3、给定时时分秒在以上集合中找到一个最小的位置 4、day+1循环直到找到满足月、星期的那一天</p>
 *
 * @author lanvendar
...
public class CronUtil {
    ...
    public static Date next(String cron, Date date) {
        ...
        return nextIncluded(cron, calendar.getTime());
    }
    
    /**
     * 5.给定cron表达式和日期，计算满足cron的下一个执行时间点，包含基础时间点.
     *
     * @param cron cron表达式
     * @param date 日期时间
     * @return 满足cron的下一个执行时间点
     */
    public static Date nextIncluded(String cron, Date date) {
        List<CronField> cronFields = convertCronField(cron);
        
        final CronField fieldSecond = cronFields.get(CronPosition.SECOND.ordinal());
        final CronField fieldMinute = cronFields.get(CronPosition.MINUTE.ordinal());
        final CronField fieldHour = cronFields.get(CronPosition.HOUR.ordinal());
        
        final CronField fieldDay = cronFields.get(CronPosition.DAY.ordinal());
        final CronField fieldMonth = cronFields.get(CronPosition.MONTH.ordinal());
        final CronField fieldWeek = cronFields.get(CronPosition.WEEK.ordinal());
     ...
     * @param fieldSecond 秒字段
     * @param fieldMinute 分字段
     * @param fieldHour   小时字段
     * @param fieldDay    天字段
     * @param fieldMonth  月字段
     * @param fieldWeek   星期字段
     * @param fieldYear   年字段
     * @return 时间
     */
    public static Date doNext(Calendar calendar, CronField fieldSecond, CronField fieldMinute, CronField fieldHour,
            CronField fieldDay, CronField fieldMonth, CronField fieldWeek, CronField fieldYear) {
        //////////////////////////////////时分秒///////////////////////////////
     ...
     * 处理日月周.
     */
    private static Date doDayOfYear(int addYear, Calendar calendar, CronField fieldDay, CronField fieldMonth,
            CronField fieldWeek, CronField fieldYear, TimeOfDay timeOfDayMin) {
     ...
     * 找到最小的满足日月周的.
     */
    private static DayOfYear findMinDayOfYear(DayOfYear dayOfYearNow, List<Integer> listDay, List<Integer> listMonth,
            CronField fieldWeek) {
     ...
     * 处理时分秒，并返回最小的时分秒.
     */
    private static TimeOfDay doTimeOfDay(Calendar calendar, CronField fieldSecond, CronField fieldMinute,
            CronField fieldHour) {
        //先确定时分秒
     ...
     *
     * @param fieldHour   小时
     * @param fieldMinute 分钟
     * @param fieldSecond 秒
     * @return 这一天的哪些时分秒执行, 不执行的返回空
     */
    private static List<TimeOfDay> timesOfDay(CronField fieldHour, CronField fieldMinute, CronField fieldSecond) {
