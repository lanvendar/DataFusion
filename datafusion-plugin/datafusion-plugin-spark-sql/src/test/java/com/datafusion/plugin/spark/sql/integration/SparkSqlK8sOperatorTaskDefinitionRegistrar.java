package com.datafusion.plugin.spark.sql.integration;
...
/**
 * 将内置 Spark SQL 作业 JSON 注册为调度系统的 Spark K8S_OPERATOR 任务定义.
 *
 * <p>直接运行 {@link #main(String[])} 时，默认处理
 * {@code plugins/spark/jobs/spark-sql-job-employee-assignment-day-example.json}。
 *
 * @author DataFusion
 * @version 1.0.0, 2026/7/13
 * @since 1.0.0
 */
public final class SparkSqlK8sOperatorTaskDefinitionRegistrar {

    /**
     * 日志对象.
     */
    private static final Logger LOGGER = Logger.getLogger(
            SparkSqlK8sOperatorTaskDefinitionRegistrar.class.getName());
    ...
    /**
     * Spark SQL 作业 JSON 资源根目录.
     */
    private static final String JOB_RESOURCE_ROOT = "plugins/spark/jobs";

    /**
     * Spark K8S_OPERATOR 任务编码前缀.
     */
     ...
     * 命令行使用说明.
     */
    private static final String USAGE = "用法：SparkSqlK8sOperatorTaskDefinitionRegistrar "
     ...
     * 默认任务类型.
     */
    private static final String DEFAULT_TASK_TYPE = "SPARK";
    ...
    /**
     * 默认 Spark SQL 作业 JSON.
     */
    private static final String DEFAULT_JOB_RESOURCE = "spark-sql-job-employee-assignment-day.json";
     ...
     * Cookie 请求头.
     */
    private final String cookie = setting("datafusion.manager.cookie",
            "DATAFUSION_MANAGER_COOKIE", DEFAULT_COOKIE);

    /**
     * 任务类型.
     */
    private final String taskType = setting("datafusion.spark.task-type",
            "DATAFUSION_SPARK_TASK_TYPE", DEFAULT_TASK_TYPE);

    /**
     * 任务类型 ID.
     */
    private final String taskTypeId = setting("datafusion.spark.task-type-id",
            "DATAFUSION_SPARK_TASK_TYPE_ID", taskType);

    /**
     * 解析后的 Spark SQL 作业 JSON classpath 资源路径.
     */
     ...
     * 创建注册工具.
     *
     * @param jobResourceInput 作业 JSON 文件名或 classpath 资源路径
     * @throws IOException 读取作业 JSON 失败
     */
    private SparkSqlK8sOperatorTaskDefinitionRegistrar(String jobResourceInput) throws IOException {
        this.jobResource = resolveJobResource(jobResourceInput);
        this.jobJson = readJobJson();
    }

    /**
     * 生成或注册 Spark K8S_OPERATOR 任务定义.
     *
     * @param args 第一个参数可以是动作，后一个参数是作业 JSON 文件名或 classpath 资源路径
     * @throws Exception 生成或注册失败
     */
    public static void main(String[] args) throws Exception {
        String action = action(args);
        SparkSqlK8sOperatorTaskDefinitionRegistrar registrar =
                new SparkSqlK8sOperatorTaskDefinitionRegistrar(jobResourceInput(args));
    ...
    private void validatePayload(JsonNode payload) {
        final JsonNode definition = payload.path("definition");
        ...
        require(definition.path("statements").isArray() && !definition.path("statements").isEmpty(),
                "Spark statements 不能为空。");
        String catalogName = definition.path("catalogName").asText();
        require(!catalogName.isBlank(), "Spark catalogName 不能为空。");
        JsonNode paimonConf = definition.path("paimonConf");
        String catalogKey = "spark.sql.catalog." + catalogName;
        require(!paimonConf.path(catalogKey).asText().isBlank(), "Spark paimonConf 缺少 " + catalogKey + "。");
        require(!paimonConf.path(catalogKey + ".warehouse").asText().isBlank(),
                "Spark paimonConf 缺少 " + catalogKey + ".warehouse。");
    }
    ...
    private ObjectNode buildRegisterPayload() {
        ...
        payload.put("description", "注册内置 Spark K8S_OPERATOR 任务定义：" + taskName());
    ...
    private JsonNode readJobJson() throws IOException {
        ...
        try (InputStream inputStream = classLoader.getResourceAsStream(jobResource)) {
            require(inputStream != null, "Spark SQL 作业资源不存在：" + jobResource);
    ...
    private String taskName() {
        ...
        return configuredName.isBlank() ? jobId() : configuredName;
    }
    ...
    private String resolveJobResource(String jobResourceInput) {
        String normalizedInput = normalizeResourcePath(jobResourceInput);
        String resourcePath = normalizedInput.startsWith(JOB_RESOURCE_ROOT + "/")
                ? normalizedInput : JOB_RESOURCE_ROOT + "/" + normalizedInput;
        require(resourceExists(resourcePath), "Spark SQL 作业资源不存在：" + resourcePath);
        return resourcePath;
    }
    ...
    private static String action(String[] args) {
        ...
        String configuredAction = setting("datafusion.spark.action", "DATAFUSION_SPARK_ACTION", DEFAULT_ACTION);
    ...
    private static String jobResourceInput(String[] args) {
        int resourceIndex = firstArgIsAction(args) ? 1 : 0;
        if (args != null && args.length > resourceIndex && !args[resourceIndex].isBlank()) {
            return args[resourceIndex];
        }
        String configuredResource = setting("datafusion.spark.job-resource",
                "DATAFUSION_SPARK_JOB_RESOURCE", DEFAULT_JOB_RESOURCE);
