...
class ConfigValidatorTest {
    ...
    void shouldAllowColumnValueOmitted() throws Exception {
        FlinkTableJobConfig config = new FlinkTableJobConfig();
        config.job.id = "job-a";
    ...
    void shouldRejectMissingStaticTableName() throws Exception {
        FlinkTableJobConfig config = baseConfig();
        PaimonTableConfig table = config.sink.tables.get(0);
        table.table.name = null;

        FlinkTableException exception = Assertions.assertThrows(FlinkTableException.class,
    ...
    private FlinkTableJobConfig baseConfig() throws Exception {
        FlinkTableJobConfig config = new FlinkTableJobConfig();
        config.job.id = "job-a";
        config.source.bootstrapServers = "localhost:9092";
        config.source.groupId = "group-a";
