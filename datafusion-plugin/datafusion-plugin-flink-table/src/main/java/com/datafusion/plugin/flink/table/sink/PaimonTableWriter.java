...
public class PaimonTableWriter implements AutoCloseable {
    ...
    public void open() {
        try {
            ...
            throw new FlinkTableException("Failed to open Paimon stream table writer: " + tableConfig.identifier(), e);
        }
    }
    ...
    public int write(List<PaimonRecord> records) {
        ...
        for (int i = 0; i < normalizedRecords.size(); i++) {
            ...
            try {
                ...
                throw new FlinkTableException("Failed to write Paimon record: " + tableConfig.identifier(), e);
            }
        }
        return written;
    }
    ...
    public PaimonCommittable prepareCommit(long commitIdentifier) {
        try {
            ...
            throw new FlinkTableException("Failed to prepare Paimon commit: " + tableConfig.identifier(), e);
        }
    }
    ...
    public void close() {
        ...
        if (write != null) {
            try {
                ...
                failure = new FlinkTableException("Failed to close Paimon stream write: " + tableConfig.identifier(), e);
            }
        }
        if (catalog != null) {
            try {
                ...
                failure = new FlinkTableException("Failed to close Paimon catalog: " + tableConfig.identifier(), e);
            }
        }
        if (failure != null) {
            throw failure;
    ...
    private void validateCreateTableSchema(Identifier tableIdentifier) {
        if (!Boolean.TRUE.equals(tableConfig.createIfNotExists)) {
            throw new FlinkTableException("Paimon table does not exist: " + tableIdentifier);
        }
        if (tableConfig.columns == null || tableConfig.columns.isEmpty()) {
            throw new FlinkTableException("Paimon create table columns is required: " + tableIdentifier);
        }
        if (tableConfig.partitionKeys == null || tableConfig.partitionKeys.isEmpty()) {
            throw new FlinkTableException("Paimon create table partitionKeys is required: " + tableIdentifier);
        }
        if (tableConfig.loadMode == LoadMode.UPSERT && (tableConfig.primaryKeys == null || tableConfig.primaryKeys.isEmpty())) {
            throw new FlinkTableException("Paimon create table primaryKeys is required for UPSERT: " + tableIdentifier);
        }
    }
        ...
        try {
            for (int i = 0; i < rowType.getFieldCount(); i++) {
                ...
                if (value == null && !field.type().isNullable()) {
                    throw new FlinkTableException("Required Paimon column is empty: " + field.name());
                }
