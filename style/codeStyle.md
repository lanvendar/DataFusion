...
Nacos Code Style file is `style/nacos-code-style-for-idea.xml` in source code. Developers can import it to Idea IDE and
reformat code by IDE.
...
2. 在checkstyle插件中设置checkstyl
3. e版本至少为8.30,并将扫描作用域设置为`All resource(including tests)`
4. 导入源代码下`style/NacosCheckStyle.xml`文件到checkstyle插件。
5. 用checkstyle插件扫描你修改的代码。
